...
def plot_train_test ( trainData, trainLabels, testData, testLabels, maxSamplesToPlot = 10000):    
    ...
    ipv.scatter(coil1TestData[:,0], coil1TestData[:,1], coil1TestData[:,2] + offset, size = 1, color='lightgreen', marker='sphere')
    ipv.scatter(coil2TestData[:,0], coil2TestData[:,1], coil2TestData[:,2] + offset, size = 1, color='purple', marker='sphere')
    ipv.pylab.squarelim()
    ipv.show()
